...
-- medium-easy tuning defaults (fallback if config missing)
local T = (Config and Config.Tuning) or {
    SearchRadius     = 40.0,    
    BiteDistance     = 3.5,     
    LureCooldownMs   = 5000,   
    BiteChance       = 0.75,   
        ...
        while fishing do
            ...
            if hasMinigameOn then
                ...
                if FISHING_GET_MINIGAME_STATE() == 6 then
                    ...
                    local fishHandle
                    for _, f in pairs(GetNearbyFishs(hookPosition, T.SearchRadius)) do
                    ...
                    if lured then
                        fishing_lure_cooldown = GetGameTimer() + T.LureCooldownMs
                    end

                    if fishHandle then
                        if math.random() < T.BiteChance then -- bite chance
                            if FISHING_GET_F_(5) == 1 then
                                ...
                                -- mark this fish as caught via rod (statebag flag)
                                pcall(function()
                                    if Entity and fishHandle and fishHandle ~= 0 then
                                        Entity(fishHandle).state:set('rsgRodCaught', true, true)
                                    end
                                end)
                                fishForce = 0.6
                ...
                if FISHING_GET_MINIGAME_STATE() == 7 then
                    ...
                    if FISHING_GET_LINE_DISTANCE() <= T.LandDistance then
                        FISHING_SET_F_(6, 12)
                        FISHING_SET_F_(14, 1.0)
                    else
                        FISHING_SET_F_(14, 1.0)
                    ...
                    if GetGameTimer() >= nextAttTime then
                        if math.random() < T.AgitatedChance then -- agitated
                            fishForce = 0.8
                    ...
                    if fishStatus == 1 then
                        local isAgitated = fishForce >= T.MinAgitatedForce
                        local ctrlHeld   = IsControlPressed(0, 0xDB096B85) -- Left Ctrl  
                        local spaceHeld  = IsControlPressed(0, 0xFBD7B3E6) -- Space (reel in)
                        local canReelForce = true

                        -- 1. Ctrl = stop gaining force (show animation only)
                        if ctrlHeld then
                            FISHING_SET_ROD_WEIGHT(4) -- show reel animation 
                            canReelForce = false
                        end

                        -- 2. Only allow force increase with Space, and only when Ctrl is not held
                        if spaceHeld and not ctrlHeld and canReelForce then
                            FISHING_SET_ROD_WEIGHT(4)
                            fishForce = fishForce + T.ForceUp
                        else
                            fishForce = fishForce - T.ForceDown
                        end

                        -- 3. Reset rod weight when neither Ctrl nor Space is held
                        if not ctrlHeld and not spaceHeld then
                            FISHING_SET_ROD_WEIGHT(2)
                        end

                        -- 4. Break / agitation clamp (only one copy)
                        if fishForce >= T.BreakThreshold then
                            FISHING_SET_F_(6, 11)
                        else
                            if fishForce < T.MinAgitatedForce then
                                fishForce = T.MinAgitatedForce
                            end
                        end
...
local function IsRodCaught(entity)
    if not entity or entity == 0 then return false end
    local ok, val = pcall(function()
        if Entity then
            local st = Entity(entity).state
            if st ~= nil then return st.rsgRodCaught == true end
        end
        return false
    end)
        ...
        if holding then
            for k, _ in pairs(Config.fishData) do
                ...
                if tonumber(heldModel) == tonumber(model) then
                    if IsRodCaught(holding) then
                        -- allow only rod-caught fish to be collected via pickup
                        ...
                        -- block world-spawn fish pickup: drop and warn
                        ClearPedTasksImmediately(ped, true, true)
                        lib.notify({ title = locale('cl_error'), description = locale('cl_only_rod_fish') or 'You can only collect fish caught with a rod.', type = 'error', duration = 4000 })
                        break
                    end
                end
            end
